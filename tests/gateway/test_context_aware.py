...
recency window, and config loading/validation of the new
`[interception]` keys.
"""
...
def test_anthropic_recency_window_protects_last_block(tmp_path):
    ...
    # Distinct row counts so each payload hashes uniquely — keeps this test
    # independent of the content-addressed dedup path on the deploy line.
    req = _anthropic_req("claude-opus", [_fat(300), _fat(400), _fat(500)])
